...
- AI 진단이 `warning` 또는 `danger`이면 이상 팝업을 자동으로 표시한다.
- 같은 설비·같은 부품의 이상이 지속되거나 반복되어도 최초 한 번으로 끝내지 않고 다시 표시한다.
- 실제 관제에서는 반복 이상을 누락하지 않는 것이 중요하지만, 이 프로젝트의 시뮬레이션은 이상이 자주 발생하므로 자동 팝업 사이에 최소 30초 간격을 둔다.
- 팝업이 이미 열려 있는 동안에는 새 팝업을 겹쳐서 열지 않는다.
