...
| 9/7(월)~9/8(화) | 1차 시스템 통합 | FastAPI↔Unity 연결, 실제 예측 결과 반영 | LightGBM 비교·최종 모델 선정 및 저장 | 실제 AI 모델 FastAPI 연결, Unity API 지원 | Jenkins CI/CD 구성, 자동 테스트·빌드 연결 |
| 9/9(수)~9/10(목) | 기능 고도화 | 상태 애니메이션·선로도·영향 센서 UI | SHAP 적용·주요 영향 센서 및 원인 분석 | SHAP 결과 API·웹 센서 차트·상태 패널 | MLflow 실험 기록·모델 버전 관리·재학습 데모 |
...
## 현재 표에서 추가 배정이 필요한 항목

강사가 요구한 Kafka 실시간 스트리밍이 현재 마일스톤에는 명시되어 있지 않다. 기존 담당을 크게 바꾸지 않는 권장 분담은 다음과 같다.
