...
def main():
    ...
    while any(s['prediction']['status']=='warming_up' for s in data.get('equipment_states',[data])) and time.monotonic()<deadline:
        time.sleep(1)
        data = fetch()
    states = data.get('equipment_states',[data])
    if data.get('source')=='remote_multi':
        assert {s['equipment_id'] for s in states}=={'station-01','station-02','station-03'}, '설비 누락'
    for state in states:
